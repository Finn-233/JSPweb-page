        ...
        for (int i = 0; i < 1; i++) {
            ...
            for (FileItem fileItem : fileItemList) {
                ...
                if (fileItem.isFormField()) {
                    ...
                    else if (fieldName.equals("phone_num")) phone_num = fileItem.getString("UTF-8").trim();//表单字段内容
...
//                msg += "<br> <a href = '" + url + "'>" + name + "</a>(" + size + "KB)";
        ...
        String sql = "";
        sql = "insert into tb_user(userid,username,password,mark,head_portrait,mailbox) values(?,?,?,'user',?,?)";
        String No = db.insert(sql, phone_num, username, password, request.getContextPath() + "/uploadFile/" + name, mailbox);
...
<%
    response.sendRedirect("index.jsp"); //登录成功网页转向
%>
</body>
</html>
